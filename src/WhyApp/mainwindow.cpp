    ...
    , ui(new Ui::MainWindow)
{
    ...
    //connect(ui->pushButton,SIGNAL())
    // 1. 初始化模型，绑定TreeView
    m_treeModel = new QStandardItemModel(this);
    // 设置表头名称
    m_treeModel->setHorizontalHeaderLabels(QStringList() << "目录结构");
...
void MainWindow::onTreeRightClick(const QPoint& pos)
{
    ...
    // 菜单点击处理
    QAction* selectedAct = menu.exec(ui->treeView->viewport()->mapToGlobal(pos));
    if (selectedAct == actEdit)
    {
        // 获取原有文字
        QStandardItem* item = m_treeModel->itemFromIndex(m_currentIndex);
    ...
    {
        m_treeModel->removeRow(m_currentIndex.row(), m_currentIndex.parent());
    }
